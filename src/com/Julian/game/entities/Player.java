...
public class Player extends Mob {
	...
	public void tick() {
		...
		if (Game.phase == 2) {
			color = Colors.get(-1, 111, 259, 444);
		} else if (Game.phase == 3) {
			color = Colors.get(-1, 112, 259, 444);
		} else if (Game.phase == 4) {
			color = Colors.get(-1, 112, 259, 543);
		}
		...
		// If the player releases the f key after interacting with it, then the
		// fCountTillRelease will reset.
		if (!input.F.isPressed() && level.getTile((this.x >> 3) + xFace, (this.y >> 3) + yFace).isInteractive()) {
			fCountTillRelease = 0;
		}
